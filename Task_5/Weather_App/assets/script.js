  ...
  fetch(
    ...
    .then((data) => {
      ...
      weather__pressure.innerHTML = `${data.main.pressure}hPA`;
        ...
        // Set the background image of the container based on the weather condition
        setContainerBackgroundImage(weatherCondition);
      });
  }
  
  // Step 3: Define a function to set the background image of the container
  function setContainerBackgroundImage(weatherCondition) {
    const container = document.querySelector('.container');
  
    // Map weather conditions to image filenames
    const conditionToImageMap = {
      'Clear': './weather_img/clear.jpg',
      'Clouds': './weather_img/cloud.jpg',
      'Rain': './weather_img/rainy.jpg',
      'Snow': './weather_img/snowy.jpg',
      // Add more mappings as needed
    };
  
    // Check if a background image exists for the weather condition
    if (conditionToImageMap.hasOwnProperty(weatherCondition)) {
      const backgroundImageURL = `url(${conditionToImageMap[weatherCondition]})`;
      container.style.backgroundImage = backgroundImageURL;
    } else {
      // Set a default background image if no match is found
      container.style.backgroundImage = 'url(default.jpg)';
    }
  }
  

// Call the getWeather function initially to fetch data for the default city
getWeather();
